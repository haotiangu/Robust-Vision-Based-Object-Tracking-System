...
class SmallImageCritic(nn.Module):
	...
	def __init__(self, image_size, state_dim, action_dim):
		...
		self.encoder = nn.Sequential(
			...
			nn.Flatten(), #,
			nn.Linear(1024, self.state_dim)  #<--- 784 is hard-coded as dependent on 448 x 448 x 3.    16 is hard-coded as dependent on 224 x 224 x 3.
		)
...
if __name__ == "__main__":
	...
	actor = SmallImageActor(image_size=(64,64), state_dim=32, action_dim=4, action_lim=1)
	x = torch.FloatTensor(np.random.rand(2, 3, 64, 64))
	action = actor.forward(x)
	print(action)

	critic = SmallImageCritic(image_size=(64, 64), state_dim=32, action_dim=4)
