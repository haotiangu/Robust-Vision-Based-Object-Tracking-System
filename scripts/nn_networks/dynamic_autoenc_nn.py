...
if __name__ == "__main__":
    ...
    n_window = 8
    dynencoder = DynamicAutoEncoderNetwork(image_size=(64, 64), action_dim=4, state_dim=16)
    image_input = torch.FloatTensor(np.random.rand(n_window, 3, 64, 64))
    ...
    pred_image_stream1 = dynencoder.decoder(output)
    dst = torch.zeros(n_window,3,64,64).long() #crop image test code the goal is expand the tensor from (1,3,112,112) to (1,3,448,448)
    dst[..., :56, :56] = pred_image_stream1
    pred_image_stream = dst
