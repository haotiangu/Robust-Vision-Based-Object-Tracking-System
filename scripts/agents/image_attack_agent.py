...
class ImageAttackTraniner:
    ...
    def make_attacked_images(self, X, Y):
        ...
        extend = torch.zeros(1,3,448,448)
        #X_adv = self.attack_network.get_attack_image(X, Y)
        X_adv = self.attack_network.get_attack_image(X, Y, extend)
        X_attacked = torch.clip(X_adv*self.alpha + X, 0, 1) #*self.alpha
...
class ImageAttacker():
    ...
    def generate_attack(self, obs, tgt_box):
        ...
        extend = torch.zeros(1,3,448,448).to(DEVICE).detach()
        """
        X: minibatch image    [(1 x 3 x 448 x 448), ...]
        Y: target coordinates [(x, y, w, h), ...]
        """
        ...
        #X_adv = self.attack_network.get_attack_image(X, Y)
        X_adv = self.attack_network.get_attack_image(X, Y, extend)
        #X_adv = self.attack_network.get_attack_image(X, Y) # this one output a tensor
        # return value of get_attack_image x = x[:, :, :self.width, :self.height] # Crop Image
        X_attacked = torch.clip(X_adv*self.alpha + X, 0, 1)
        X_ATTACKED = X_attacked
        X_ADV = torch.clip(X_adv, 0, 1) 
        return X_attacked.detach().squeeze().permute(1, 2, 0).cpu().numpy(), X_ATTACKED, X_ADV.detach().squeeze().permute(1, 2, 0).cpu().numpy(), X_ADV