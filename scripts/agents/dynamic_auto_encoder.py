...
class DynamicAutoEncoderAgent:
    ...
    def predict_batch_images(self, stream_arr, state_est_arr, tgt_arr):
        ...
        pred_image_stream1 = self.nn_model.decoder(output)
        #pred_image_stream = pred_image_stream[:, :, :self.nn_model.width, :self.nn_model.height] # this crop image is not feasible
        goal = torch.zeros(n_window,3,self.nn_model.width,self.nn_model.height).to(DEVICE)
        goal[..., :56, :56] = pred_image_stream1
        pred_image_stream = goal
