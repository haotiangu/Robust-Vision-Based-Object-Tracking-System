...
from FPRN.models.autoencoder_model import AutoencoderModel
from FPRN.models.autoencoder_model import AutoencoderModel


def test_srnet_model():
    with open('tests/data/test_SRNet_model.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    # build model
    model = SRNetModel(opt)
    # test attributes
    assert model.__class__.__name__ == 'SRNetModel'
...
def test_autoencoder_model():
    with open('tests/data/test_autoencoder_model.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    # build model
    model = AutoencoderModel(opt)
    # test attributes
    assert model.__class__.__name__ == 'AutoencoderModel'
    assert isinstance(model.net_g, SRNet)  # generator
    assert isinstance(model.net_d, Autoencoder)  # discriminator
    assert isinstance(model.cri_pix, L1Loss)
