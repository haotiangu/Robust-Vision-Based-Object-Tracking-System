...
from FPRN.data.autoencoder_dataset import AutoencoderDataset
from FPRN.data.autoencoder_paired_dataset import AutoencoderPairedDataset
def test_autoencoder_dataset():

    with open('tests/data/test_autoencoder_dataset.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    dataset = AutoencoderDataset(opt)
...
def test_autoencoder_paired_dataset():

    with open('tests/data/test_autoencoder_paired_dataset.yml', mode='r') as f:
        opt = yaml.load(f, Loader=yaml.FullLoader)

    dataset = AutoencoderPairedDataset(opt)
    assert dataset.io_backend_opt['type'] == 'disk'  # io backend
    assert len(dataset) == 2  # whether to read correct meta info
